...
import co.casterlabs.quark.ingest.QuarkIngest;
import co.casterlabs.quark.ingest.util.SocketConnection;
import xyz.e3ndr.fastloggingframework.logging.FastLogger;

class _RTMPConnection implements AutoCloseable, SessionListener {
...
//        if (read.message() instanceof RTMPMessageData0 data) {
...
//                    playbackMuxer.write(new FLVTag(FLVTagType.SCRIPT, 0, 0, payload));
//                }
//            }
//            return;
//        }
    ...
    private void handleCommand(RTMPChunk<RTMPMessageCommand0> read) throws IOException {
        ...
        switch (read.message().commandName().value()) {
            ...
            case "publish":
                ...
                this.session = QuarkIngest.authenticateSession(this, this.handshakeUrl, key);
...
//        this.logger.trace("Audio packet: %s", read);
        ...
        if (tag.data().isSequenceHeader()) {
            ...
            this.sequenceTags.add(seq);
            this.session.data(seq);
        } else {
...
//        this.logger.trace("Video packet: %s", read);
    ...
    public void onSequenceRequest(QuarkSession session) {
        for (FLVSequenceTag seq : this.sequenceTags) {
            session.data(seq);
        }
    }
